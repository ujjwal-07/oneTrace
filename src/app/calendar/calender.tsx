...
const page = () => {
    ...
    let date = ''
    const lightColors = [
      "bg-pink-100 border-[#FF847E]  text-black",
      "bg-[#F2FAFF] border-[#7EC1FF]  text-black",
      "bg-[#FFF7E2] border-[#FFD35D]  text-black",
      "bg-[#F6FFED] border-[#AEFF5D] text-black"]
  ...
  <div className="rounded-[5px] h-[21px] w-[35px] bg-[#FF625B] absolute top-[3.5625rem] left-[0.0625rem] grid place-items-center text-white text-sm">
  ...
  {selectedDate && eventDatastored.length > 0 ? (
    ...
    <ul className=" space-y-3">
      {eventDatastored.map((event, index) => {
        const randomColor = lightColors[index % lightColors.length]; // Assign color based on index for consistency
        ...
        return (
          ...
          >
            <span className="font-medium ">{index + 1}. {event}</span>
            <button
              className="text-gray-600 hover:text-gray-800 p-2 text-lg"
...
>
  ...
  <form onSubmit={handleSubmit} className="flex items-center w-full">
    ...
    <button
      type="submit"
      className=" p-1 bg-blue-500 text-white hover:bg-blue-600 flex items-center justify-center w-12 h-[2rem] rounded-lg"
    >
      Add
    </button>
  </form>
